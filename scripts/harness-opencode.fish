#!/usr/bin/env fish
# harness-opencode.fish — Symlink harness teams into OpenCode config.
#
# Three mappings per team:
#   harness/<team>/teams/*.md       →  .opencode/agents/<team>/*.md
#   harness/<team>/skills/*/        →  .opencode/skills/*
#   harness/<team>/<team>.md        →  .opencode/instructions/<team>.md
#
# Usage:
#   ./scripts/harness-opencode.fish link <team>              # link one team
#   ./scripts/harness-opencode.fish unlink <team>            # unlink one team
#   ./scripts/harness-opencode.fish link <team> --dry-run    # preview
#   ./scripts/harness-opencode.fish unlink <team> --dry-run  # preview
#   ./scripts/harness-opencode.fish link <team> --force      # overwrite conflicts
#   ./scripts/harness-opencode.fish unlink <team> --force    # force unlink (only symlinks)
#   ./scripts/harness-opencode.fish link --all               # link all teams
#   ./scripts/harness-opencode.fish unlink --all             # unlink all teams

set -g SCRIPT_DIR (dirname (status filename))
set -g PROJECT_ROOT (realpath "$SCRIPT_DIR/..")

# ── helpers ────────────────────────────────────────────────────────────────

function _usage
    echo "Usage:"
    echo "  harness-opencode.fish link <team>              Link a harness team into OpenCode"
    echo "  harness-opencode.fish unlink <team>            Unlink a harness team from OpenCode"
    echo "  harness-opencode.fish link <team> --dry-run    Preview link actions"
    echo "  harness-opencode.fish unlink <team> --dry-run  Preview unlink actions"
    echo "  harness-opencode.fish link <team> --force      Link, replacing conflicts"
    echo "  harness-opencode.fish unlink <team> --force    Unlink, force-remove matching symlinks"
    echo "  harness-opencode.fish link --all               Link every valid harness team"
    echo "  harness-opencode.fish unlink --all             Unlink every valid harness team"
    exit 1
end

function _die
    echo "ERROR: $argv" >&2
    exit 1
end

function _warn
    echo "WARN:  $argv" >&2
end

function _relpath_py
    # Relative path from $argv[1] (from_abs_dir) to $argv[2] (to_abs_path).
    # Uses python3. Handles non-existent from_dir by walking up ancestors.
    set -l from "$argv[1]"
    set -l to "$argv[2]"
    python3 -c "
import os
f = '$from'
t = '$to'
while f and not os.path.isdir(f):
    f = os.path.dirname(f) or '/'
print(os.path.relpath(t, f))
"
end

function _resolve_link_target
    # Return a relative symlink target for $argv[1] (dst) → $argv[2] (src).
    set -l dst "$argv[1]"
    set -l src "$argv[2]"
    set -l dst_dir (dirname "$dst")
    set -l src_abs (realpath "$src")
    _relpath_py "$dst_dir" "$src_abs"
end

function _is_symlink
    test -L "$argv[1]"
end

function _realpath_link
    # Resolve a symlink's target relative to the symlink's directory.
    # Handles absolute targets (starts with /) directly.
    set -l link "$argv[1]"
    set -l target (readlink "$link")
    if string match -q '/*' "$target"
        realpath "$target"
    else
        set -l dir (dirname "$link")
        realpath "$dir/$target"
    end
end

function _is_symlink_to
    # Check if $path is a symlink pointing to $target (canonical comparison).
    _is_symlink "$argv[1]"; or return 1
    set -l actual (_realpath_link "$argv[1]")
    set -l expected (realpath "$argv[2]")
    test "$actual" = "$expected"
end

function _list_teams
    # Find dirs under harness/ that contain both teams/ and skills/
    set -l harness_dir "$PROJECT_ROOT/harness"
    test -d "$harness_dir"; or return
    for d in "$harness_dir"/*/
        set -l name (basename "$d")
        test -d "$d/teams" -a -d "$d/skills"; and echo "$name"
    end
end

function _validate_team
    set -l team $argv[1]
    set -l team_dir "$PROJECT_ROOT/harness/$team"
    test -d "$team_dir"; or _die "harness/$team does not exist"
    test -d "$team_dir/teams"; or _die "harness/$team/teams does not exist"
end

# ── counters ────────────────────────────────────────────────────────────────

set -g _linked 0
set -g _unlinked 0
set -g _skipped 0
set -g _warnings 0
set -g _errors 0

function _reset_counters
    set -g _linked 0
    set -g _unlinked 0
    set -g _skipped 0
    set -g _warnings 0
    set -g _errors 0
end

function _print_summary
    echo ""
    echo "────────── Summary ──────────"
    echo "  linked:   $_linked"
    echo "  unlinked: $_unlinked"
    echo "  skipped:  $_skipped"
    echo "  warnings: $_warnings"
    echo "  errors:   $_errors"
    echo "──────────────────────────────"
end

# ── link helpers ────────────────────────────────────────────────────────────

function _link_one
    # Symlink $src → $dst with label $label. Returns via counters.
    set -l src "$argv[1]"
    set -l dst "$argv[2]"
    set -l label "$argv[3]"
    set -l force "$argv[4]"
    set -l dry_run "$argv[5]"

    set -l rel_target (_resolve_link_target "$dst" "$src")

    if _is_symlink_to "$dst" "$src"
        echo "  SKIP   $label (already linked)"
        set -g _skipped (math $_skipped + 1)
        return
    end

    if test -e "$dst" -o -L "$dst"
        if test "$force" = true
            if test "$dry_run" = true
                echo "  [dry-run] would replace: $label"
                set -g _linked (math $_linked + 1)
            else
                echo "  FORCE  $label (replacing existing)"
                rm -rf "$dst"
                ln -s "$rel_target" "$dst"
                set -g _linked (math $_linked + 1)
            end
        else
            _warn "$label exists (use --force to replace)"
            set -g _warnings (math $_warnings + 1)
            set -g _skipped (math $_skipped + 1)
        end
        return
    end

    if test "$dry_run" = true
        echo "  [dry-run] would link: $label"
        set -g _linked (math $_linked + 1)
    else
        echo "  LINK   $label"
        mkdir -p (dirname "$dst")
        ln -s "$rel_target" "$dst"
        set -g _linked (math $_linked + 1)
    end
end

function _unlink_one
    # Remove symlink at $dst if it points to $src. Label for display.
    set -l src "$argv[1]"
    set -l dst "$argv[2]"
    set -l label "$argv[3]"
    set -l dry_run "$argv[4]"

    if not test -e "$dst" -o -L "$dst"
        return
    end

    if not _is_symlink "$dst"
        _warn "$label is not a symlink — skipping"
        set -g _warnings (math $_warnings + 1)
        set -g _skipped (math $_skipped + 1)
        return
    end

    set -l link_target (_realpath_link "$dst" 2>/dev/null)
    set -l expected_src (realpath "$src" 2>/dev/null)

    if test -z "$link_target" -o -z "$expected_src" -o "$link_target" != "$expected_src"
        return
    end

    if test "$dry_run" = true
        echo "  [dry-run] would remove: $label"
        set -g _unlinked (math $_unlinked + 1)
    else
        echo "  UNLINK $label"
        rm "$dst"
        set -g _unlinked (math $_unlinked + 1)
    end
end

# ── link team ───────────────────────────────────────────────────────────────

function _link_team
    set -l team $argv[1]
    set -l force "$argv[2]"
    set -l dry_run "$argv[3]"

    _validate_team "$team"
    echo "── Linking harness/$team ──"

    set -l team_dir "$PROJECT_ROOT/harness/$team"

    # 1. Agents: harness/<team>/teams/*.md -> .opencode/agents/<team>/*.md
    set -l agents_dir "$PROJECT_ROOT/.opencode/agents/$team"
    if test "$dry_run" = true
        echo "[dry-run] would create dir: .opencode/agents/$team"
    else
        mkdir -p "$agents_dir"
    end
    for src_file in "$team_dir/teams/"*.md
        test -f "$src_file"; or continue
        set -l fname (basename "$src_file")
        _link_one "$src_file" "$agents_dir/$fname" "agents/$team/$fname" "$force" "$dry_run"
    end

    # 2. Skills: harness/<team>/skills/*/ -> .opencode/skills/*
    set -l skills_src "$team_dir/skills"
    for src_item in "$skills_src"/*
        test -e "$src_item"; or continue
        set -l sname (basename "$src_item")
        _link_one "$src_item" "$PROJECT_ROOT/.opencode/skills/$sname" "skills/$sname" "$force" "$dry_run"
    end

    # 3. Instruction: harness/<team>/<team>.md -> .opencode/instructions/<team>.md
    set -l ins_src "$team_dir/$team.md"
    if test -f "$ins_src"
        _link_one "$ins_src" "$PROJECT_ROOT/.opencode/instructions/$team.md" "instructions/$team.md" "$force" "$dry_run"
    end
end

# ── unlink team ─────────────────────────────────────────────────────────────

function _unlink_team
    set -l team $argv[1]
    set -l force "$argv[2]"
    set -l dry_run "$argv[3]"

    _validate_team "$team"
    echo "── Unlinking harness/$team ──"

    set -l team_dir "$PROJECT_ROOT/harness/$team"
    set -l agents_dir "$PROJECT_ROOT/.opencode/agents/$team"

    # 1. Unlink agents
    if test -d "$agents_dir"
        for dst_item in "$agents_dir"/*
            test -e "$dst_item" -o -L "$dst_item"; or continue
            set -l dname (basename "$dst_item")
            _unlink_one "$team_dir/teams/$dname" "$dst_item" "agents/$team/$dname" "$dry_run"
        end
        # Remove empty agents dir
        set -l remaining (ls -A "$agents_dir" 2>/dev/null)
        if test -z "$remaining"
            if test "$dry_run" = true
                echo "  [dry-run] would remove empty dir: .opencode/agents/$team"
            else
                echo "  RMDIR  .opencode/agents/$team (empty)"
                rmdir "$agents_dir"
            end
        end
    else
        echo "  (no agents dir to unlink: .opencode/agents/$team)"
    end

    # 2. Unlink skills
    set -l skills_dst "$PROJECT_ROOT/.opencode/skills"
    if test -d "$skills_dst"
        for dst_item in "$skills_dst"/*
            test -e "$dst_item" -o -L "$dst_item"; or continue
            set -l sname (basename "$dst_item")
            _unlink_one "$team_dir/skills/$sname" "$dst_item" "skills/$sname" "$dry_run"
        end
    end

    # 3. Unlink instruction
    _unlink_one "$team_dir/$team.md" "$PROJECT_ROOT/.opencode/instructions/$team.md" "instructions/$team.md" "$dry_run"
end

# ── main dispatch ───────────────────────────────────────────────────────────

function _main
    set -l cmd ""
    set -l team ""
    set -l all false
    set -l force false
    set -l dry_run false

    for arg in $argv
        switch $arg
            case link unlink
                set cmd $arg
            case --all -a
                set all true
            case --force -f
                set force true
            case --dry-run -n
                set dry_run true
            case --help -h
                _usage
            case '-*'
                _die "Unknown flag: $arg"
            case '*'
                if test -z "$team"
                    set team $arg
                else
                    _die "Unexpected argument: $arg"
                end
        end
    end

    test -n "$cmd"; or _usage

    set -l teams
    if test "$all" = true
        set teams (_list_teams)
        if test -z "$teams"
            _die "No valid harness teams found (need harness/<team>/teams/ and harness/<team>/skills/)"
        end
        echo "Teams found: "(string join ", " $teams)
    else
        test -n "$team"; or _die "Specify a team name, or use --all"
        set teams "$team"
    end

    cd "$PROJECT_ROOT" 2>/dev/null; or _die "Cannot enter project root: $PROJECT_ROOT"

    set -l cmd_label
    if test "$cmd" = link
        set cmd_label "link"
    else
        set cmd_label "unlink"
    end
    if test "$dry_run" = true
        set cmd_label "$cmd_label (dry-run)"
    end

    _reset_counters

    echo "Command:  $cmd_label"
    if test "$all" = true
        echo "Teams:    all"
    else
        echo "Team:     $team"
    end
    echo ""

    for t in $teams
        if test "$cmd" = link
            _link_team "$t" "$force" "$dry_run"
        else
            _unlink_team "$t" "$force" "$dry_run"
        end
        echo ""
    end

    _print_summary

    if test $_errors -gt 0
        exit 1
    end
end

_main $argv
