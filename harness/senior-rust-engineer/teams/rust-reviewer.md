...
- Verify `unsafe` blocks: every one must have a `// SAFETY:` comment explaining the invariant.
- Check lock ordering for deadlock potential. Verify no locks held across `.await`.
...
- **Output:** Review findings at `_workspace/04_review_findings.md` — categorized as BLOCKER (must fix), WARNING (should fix), INFO (consider).
