...
- Migrating a crate to the Rust 2024 edition
...
- **Resolver v3 by default.** Edition 2024 implies `resolver = "3"` (MSRV-aware dependency resolution). In a workspace, set `resolver = "3"` on the root `[workspace]`.
- **RPIT precise capturing.** Return-position `impl Trait` now captures **all** in-scope generic parameters and lifetimes by default. Opt out with a `use<>` bound, e.g. `-> impl Trait + use<T>` to capture `T` but not the lifetimes in scope.
- **`unsafe extern` blocks.** `extern` blocks must be written `unsafe extern "C" { ... }`; individual items can be marked `safe` or `unsafe`.
- **Unsafe attributes.** `no_mangle`, `export_name`, and `link_section` must be wrapped: `#[unsafe(no_mangle)]`.
- **`unsafe_op_in_unsafe_fn` warns by default.** Wrap unsafe operations inside an `unsafe fn` in an explicit `unsafe { }` block.
- **References to `static mut` are rejected** (`static_mut_refs` is deny-level). Use `&raw const` / `&raw mut`, atomics, or a safe wrapper instead.
- **Prelude additions.** `Future` and `IntoFuture` are in the 2024 prelude — drop now-redundant imports and watch for name clashes.
- **`gen` is a reserved keyword** (for future generator blocks); rename any identifier named `gen`.
- **Tail-expression / temporary drop order changed.** Temporaries in a block's tail expression and in `if let` scrutinees drop earlier; review code that relied on the old timing (e.g. a lock guard held across a tail expression).
- **Rustfmt style edition.** Set `style_edition = "2024"` in `rustfmt.toml` for the current formatting style.

For the authoritative, complete list, consult the official Rust Edition Guide (2024 chapter). Everything below applies across editions; prefer the 2024 forms above where they differ.
