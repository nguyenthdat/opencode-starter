...
> **Harness adaptation (opencode):** originated as a Claude Code plugin. In the swe-team harness the `swe-team` orchestrator is the orchestrator and spawns its own `task` subagents — there is no `/rust-review:rust-review` slash command or auto-registered `rust-review:*` plugin subagents. The worker/judge prompts live in `agents-workers/*.md` and are used as task-prompt bodies. Set the plugin-root variable to this skill's own directory so the scripts resolve: `RUST_REVIEW_PLUGIN_ROOT=.opencode/skills/rust-review` (it holds `prompts/` and `scripts/`). Even without the automated pipeline, the `prompts/` tree (60+ Rust vulnerability finders + clusters) is a first-class security-review checklist library — apply the relevant finders directly.
