...
Write instruction notes for a reader with zero context: the assistant loading it next month is effectively a stranger. Explain _why_ behind rules that aren't obvious — an assistant that understands the reason applies the rule correctly in situations the rule didn't anticipate.
...
For a light setup, changelogs on _instruction notes, schemas, and indexes_ are the non-negotiable core; changelogs on every individual content note are optional.
