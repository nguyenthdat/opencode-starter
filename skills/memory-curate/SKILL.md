...
This skill curates the **knowledge graph** — the notes, relations, and tags that make up the knowledge base. (For hygiene on an agent's own memory _files_ — splitting bloated files, pruning stale entries — see **memory-defrag**.)
...
4. **Explain suggestions.** Say _why_ a relation or merge makes sense.
